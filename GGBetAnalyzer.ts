...
const mem = {}; // fallback if window.storage unavailable
async function loadKey(key, fallback) {
  try {
    if (window?.storage?.get) { const r = await window.storage.get(key); return r ? JSON.parse(r.value) : fallback; }
  } catch (_) {}
  return key in mem ? mem[key] : fallback;
}
async function saveKey(key, val) {
  mem[key] = val;
  try { if (window?.storage?.set) await window.storage.set(key, JSON.stringify(val)); } catch (e) { /* noop */ }
}
