    ...
    <div style={{ minHeight: "100vh", background: C.bg, color: C.text, fontFamily: FONT.ui }}>
      ...
      <aside className="ggba-sidebar">
        <div style={{ display: "flex", alignItems: "center", gap: SP.md, padding: `${SP.lg}px ${SP.lg}px ${SP.md}px` }}>
          <div style={{ width: 32, height: 32, borderRadius: RADIUS.md, background: C.accent, color: "#100c22",
      ...
      <main className="ggba-main">
        ...
        {loaded && players.length < 2 && tab === "analyze" && <div style={{ background: C.surface, border: `1px solid ${C.border}`, borderRadius: 10, padding: 16, marginBottom: 16 }}>
          ...
          <button onClick={() => setTab("data")} style={{ background: C.accent, color: "#100c22", border: 0, borderRadius: 8, padding: "12px 18px", fontWeight: 800 }}>Load league data</button>
        </div>}
        {loaded && <Automation setPlayers={setPlayers} setMatches={setMatches} setWf={setWf} setBets={setBets} />}
        {!loaded ? (
